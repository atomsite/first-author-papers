...
Despite this dust-hostile environment, CWB systems containing a Wolf-Rayet carbon phase star (WC) have been observed producing copious quantities of dust (so-called WCd systems; see \cite{Williams1995}).
...
The scalar quantities tracked in this model are:

\begin{itemize}
  \item $C$, the wind ``colour'', or mixing fraction (see Eq. \ref{eq:p2-c}).
  \item $z$, the dust-to-gas mass fraction (see Eq. \ref{eq:p2-z}).
  \item $a$, the grain radius in microns.
\end{itemize}

\noindent
As there is no diffusion coefficient in Eq. \ref{eq:p2-scal}, these quantities can be interpreted as explicitly co-moving with the fluid.
...
\begin{subequations}
  \begin{align}
    \rho_R & = \frac{\mdot}{4 \pi r^2 v_R} , \\
    % P_R    & = \frac{\rho_R}{\mu m_H} k_B T_w , \\
    p_R    & = \rho_R v_{R} , \\
    E_R    & = \frac{P_R}{\gamma - 1} + \frac{1}{2} p_R v_R ,
  \end{align}
\end{subequations}
...
In the case of systems with a very small orbital separation this could result in collision with a higher than anticipated wind velocity.
...
The lookup table was generated by mixing a series of cooling curves from a MEKAL calculation of elemental gasses \citep{mewe1995update}.
...
Dust grains are assumed to have a single size for each grid cell.
The number density of grains varies in the simulation volume, however the ratio of the grain number density is fixed and constant throughout the simulation.
...
\begin{equation}
  \frac{da\rms{sp}}{dt} = - \frac{a}{\tau\rms{gr}} = - \num{1.056e-18} \cdot n\rms{g}. 
\end{equation}
...
In the case of WR140, $\dsep$ varies by a factor of 19 from apastron to periastron.
...
\begin{table}
  ...
  \hline
  \end{tabular}
  \caption[Abundances by mass used for OB and WR stars]{Abundances used for the OB and WR stars being simulated. Other elements are assumed to be trace when calculating dust emission \citep{williamsSpectraWC9Stars2015,sanderGalacticWCStars2012,andersAbundancesElementsMeteoritic1989}}.
...
\begin{figure}
  ...
  \caption[A graph of the overall dust mass in the simulation of WR140 over the orbital phase $0.95 \leq \Phi \leq 1.10$]{A graph of the overall dust mass in the simulation of WR140 over the orbital phase $0.95 \leq \Phi \leq 1.10$. The amount of dust quickly reduces after periastron due to a decreased dust growth rate (Fig. \ref{fig:wr140-dustproduction}), as well as dust advecting off the numerical grid. While we observe a double peak in the dust production rate, this is due to a high level of grain ablation occurring as well as grain growth.}
...
This is consistent with $L^\prime$ photometry of WR140 \citep{crowther_dust_2003} -- the photometry magnitude sharply increases over the first year -- and suggests our model can qualitatively assess dust production rates of episodic WCd systems.
...
In the case of Fig. \ref{fig:wr140-dustmass} we observe a double peak structure between $\Phi = 1.00$ and $\Phi = 1.02$.
At the peak rate of dust formation over this period, we would observe a dust mass in the simulation approaching \SI{6e-8}{\solarmass} if dust destruction were not included in the simulation.
As such, we can infer that the peak structure is due to a pronounced increase in the dust destruction rate as the post-shock density and temperature are at their highest during the systems periastron passage.
...
These results are also in agreement with more contemporary research by \cite{pollockCompetitiveXRayOptical2021} which notes a similar sharp increase in K-band flux and x-ray column density a short time after periastron passage.
...
Finally, we would like to thank the academic referee for their extremely helpful review.
